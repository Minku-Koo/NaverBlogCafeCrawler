...
def getURLs(
    ...
    while True:
        URL = {
            ...
            "cafe": f"""https://s.search.naver.com/p/cafe/search.naver?where=article&ie=utf8&query={keyword}&prdtype=0&t=0&st=rel&srchby=text&dup_remove=1&cafe_url=&without_cafe_url=&sm=tab_opt&nso=so:r,p:from{start_date}to{end_date},a:all&nso_open=0&rev=44&abuse=0&ac=0&aq=0&converted=0&is_dst=0&nqx_context=&nx_and_query=&nx_search_hlquery=&nx_search_query=&nx_sub_query=&people_sql=0&spq=0&x_tab_article=&is_person=0&start={page}&display=10&prmore=1&_callback=viewMoreContents"""
        ...
        if len(results) <= 1 or results[-1].text == temp:
            break
